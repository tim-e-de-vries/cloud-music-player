...
const DRIVE_FOLDER_ID = '19rCsfU1J6fJdIynDh73kPimkce1tXlxj';
  ...
  corsMiddleware(req, res, async () => {
    try {
      ...
      const fileList = await drive.files.list({
        q: `'${DRIVE_FOLDER_ID}' in parents and (mimeType='audio/mpeg' or mimeType='audio/flac' or mimeType='audio/x-flac' or mimeType='audio/wav' or mimeType='application/vnd.google-apps.folder')`,
        fields: 'files(id, name, mimeType, size)',
//        pageToken: pageToken,
        orderBy: 'name', // Sort files alphabetically
        pageSize: 50 // Increased page size for efficiency
      });
